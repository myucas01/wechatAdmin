<template>
  <div class="app-container">
        ...
        <div class="appWrap">
          ...
          >
            ...
            <!-- <el-form-item label="登录密码: " prop="password" style="width:80%">
              <el-input value="xxxxx" type="password" disabled="true" placeholder="请设置登录密码" />
            </el-form-item>-->
            <el-form-item label="登录密码: " prop="password" style="width:80%">
              <el-input value="12345" type="password" placeholder="请设置登录密码" />
          ...
          >
            ...
            <el-form-item label="归属企业" prop="parentCompany" style="width:75%">
              <el-select style="width: 100%;"  ref="parentCompany" v-model="ruleForm.parentCompany" :remote-method="remoteMethod"
    ...
    <div style="position: absolute;bottom: 0;left: 45%;width: 100%;height: 60px;line-height: 60px;">
      ...
      <el-button style="margin-left:50px" :loading="qcLoading" type="primary" @click="submitForm">修改</el-button>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    // this.$nextTick(()=>{
    //   console.log(this.ruleForm.parentCompany)
    //   this.ruleForm.companyName = this.$refs.parentCompany.selectedLabel
    // })
  },
  watch: {
    ruleForm: {
  ...
  methods: {
          ...
          companyList(this.listQuery).then(response => {
            ...
            this.list.forEach(item => {
              if (item.value == this.ruleForm.companyId) {
                this.options = item;
                this.ruleForm.parentCompany = item.label;
              }
            });
          });
        }
    ...
    handleCompany(e) {
      ...
      console.log(this.$refs.parentCompany.query)
      console.log(this.$refs.parentCompany)

       let obj = {};
      obj = this.list.find((item)=>{//这里的userList就是上面遍历的数据源
    ...
    submitForm() {
      this.ruleForm.action = '编辑企业账号'
      this.ruleForm.id = this.$route.query.id;
      this.ruleForm.companyId = this.$route.query.companyId;

      companyUserEdit(this.ruleForm).then(res => {
        console.log(res);
        if (res.data.errno == 0) {
